...
## MantaRayss (GLMM with negative binomial error function)
MantaRays_ZINB_GLMM_full_model <- glmmadmb(formula=MantaRays~StudyYear+CurrentCode+SeaTempCelsius+VisibilityMeters+ElNinoIndex+SIN_TIME+COS_TIME+(1|DiverCode)+(1|SiteCode),data=CocosData,family="nbinom",extra.args="-ndi 30000",zeroInflation=TRUE)
MantaRays_ZINB_GLMM_mumin <- dredge(MantaRays_ZINB_GLMM_full_model,rank="AIC",subset=(COS_TIME | !SIN_TIME) && (!COS_TIME | SIN_TIME))
...
save(MantaRays_ZINB_GLMM_mumin,file='/home/erwhite1/data/MantaRays_ZINB_GLMM_mumin.Rdata')
